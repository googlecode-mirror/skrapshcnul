...
function generate_profile_card_html(userid, callback) {
	...
	jQuery(".ls-profile-card-loader-overlay").show();
	jQuery(".ls-profile-card-content").hide();
	
	if (jQuery(".ls-profile-card-content").attr('ls-oid') != userid) {
			...
			if (data.results) {
				...
				var return_html = 
					...
					'<div class="profile-info">'+
						...
						'</div>'+
					'</div>'
				jQuery(".ls-profile-card-content").html(jQuery(return_html));
				jQuery(".ls-profile-card-content").attr('ls-oid', userid);
