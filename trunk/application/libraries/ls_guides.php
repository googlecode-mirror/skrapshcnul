...
class Ls_Guides {
	...
	function getWelcomeGuide_States($user_id = FALSE) {
		...
		$count = array();
		$count['pending_requests'] = $this -> ci -> ls_events -> countPendingEventRequests($user_id);
		$count['pending_recommendations'] = intval($this -> ci -> ls_user_recommendation -> countPendingUserRecommendations($user_id));

		if ($count['pending_requests'] == 0 && $count['pending_recommendations'] == 0) {
			$result['step4']['state'] = "Completed";
		} else {
			$result['step4']['state'] = "Incomplete";
			$result['step4']['reason'] = ($count);
		}
